...
class MoviesSpider(scrapy.Spider):
    name = 'movies'
    allowed_domains = ['film2serial.ir']

    def parse(self, response):
        # posts = response.xpath('//article[@class="post"]/div[1][contains(@class , "entry")]/header/h3/a/@href').getall()
        posts = response.xpath('//*[contains(concat( " ", @class, " " ), concat( " ", "more", " " ))]//a/@href').getall()
        ...
        try:
            ...
            if current_page < last_page:
                ...
                split_url[-1] = next_page_url
                absolute_next_page_url = '/'.join(str(e) for e in split_url)
                # absolute_next_page_url = base_url + '/page/{}'.format(next_page_url)
            ...
            absolute_next_page_url = response.request.url + '/page/2'
            yield {'abs url': response.request.url}
            page_counter = page_counter - 1
            yield scrapy.Request(url=absolute_next_page_url , meta = {'page_counter':page_counter})
    ...
    def parse_page(self,response):
        try:
            ...
            summary = response.xpath("//*[contains(text(),'خلاصه داستان')]/..").get()
            summary_root = lxml.html.fromstring(summary)
            summary_element = lxml.html.tostring(summary_root,method='text',encoding='unicode')
            summary = summary_element
            summary = summary.replace('خلاصه داستان :','').replace('\n','')
            ...
            urls = response.xpath('/html/body/div[2]/div[1]/div[1]/div[2]/div[3]/p/span/a/@href').getall()
